...
export default class RoomClient {
  ...
  async changeDisplayName(displayName) {
    ...
    try {
      await this.sendRequest('changeDisplayName', {
        displayName
      });
  ...
  async changePicture(picture) {
    ...
    try {
      await this.sendRequest('changePicture', {
        picture
      });
    } catch (error) {
  ...
  async sendChatMessage(chatMessage) {
    ...
    try {
      ...
      await this.sendRequest('chatMessage', {
        chatMessage
      });
    } catch (error) {
  ...
  async _sendFile(magnetUri) {
    ...
    try {
      await this.sendRequest('sendFile', {
        magnetUri
      });
    } catch (error) {
  ...
  async changeAudioDevice(deviceId) {
    ...
    try {
      ...
      if (this._micProducer)
        await this._micProducer.replaceTrack({
          track
        });
  ...
  async promoteLobbyPeer(peerId) {
    ...
    try {
      await this.sendRequest('promotePeer', {
        peerId
      });
    } catch (error) {
  ...
  async sendRaiseHandState(state) {
    ...
    try {
      await this.sendRequest('raiseHand', {
        raiseHandState: state
      });
  ...
  async requestConsumerKeyFrame(consumerId) {
    ...
    try {
      await this.sendRequest('requestConsumerKeyFrame', {
        consumerId
      });
    } catch (error) {
  ...
  async setAccessCode(code) {
    ...
    try {
      await this.sendRequest('setAccessCode', {
        accessCode: code
      });
